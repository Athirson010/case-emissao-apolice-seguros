...
Sistema robusto e escalável para emissão, gerenciamento e processamento de apólices de seguros, desenvolvido com foco em
**Clean Architecture**, **Event-Driven Architecture** e boas práticas de desenvolvimento (SOLID, Clean Code).
...
- **Spring Data MongoDB** - Integração Spring com MongoDB
- **RabbitMQ 3.13** - Message broker para mensageria assíncrona
- **Spring AMQP (Spring Rabbit)** - Integração Spring com RabbitMQ
- **Apache Kafka 7.5.0** - Plataforma de streaming de eventos
- **Apache Zookeeper** - Coordenação para Kafka
- **Spring Kafka 3.1.1** - Integração Spring com Kafka

#### API e Validação

- **Spring Web (MVC)** - REST API controllers
- **Bean Validation (Jakarta Validation)** - Validação de dados (@Valid, @NotNull, etc.)
- **Jackson** - Serialização/deserialização JSON
- **SpringDoc OpenAPI 2.3.0** - Documentação interativa da API (Swagger UI)
- **Swagger Annotations Jakarta 2.2.19** - Anotações para documentação de API

#### Observabilidade (Grafana Stack - LGTM)

- **Grafana 10.2.3** - Dashboards e visualização unificada
- **Loki 2.9.3** - Agregação e indexação de logs
- **Tempo 2.3.1** - Distributed tracing
- **Prometheus 2.48.1** - Coleta de métricas
- **Promtail 2.9.3** - Agent de coleta de logs
- **Micrometer** - Instrumentação de métricas (Prometheus integration)
- **OpenTelemetry** - Coleta de traces distribuídos (OTLP exporter)
- **Loki4j 1.5.1** - Loki Logback Appender para envio de logs

#### Qualidade e Testes

- **JUnit 5 (Jupiter) 5.10.1** - Framework de testes unitários
...
- **ArchUnit 1.2.1** - Testes de arquitetura e regras estruturais
- **JaCoCo 0.8.11** - Cobertura de código
- **SonarQube 10.3 Community** - Análise de qualidade de código
- **PostgreSQL 15** - Banco de dados para SonarQube
...
- **Domain-Driven Design (DDD)** - Modelagem de domínio rico
...
A apólice utiliza o conceito de **Rejeição Imediata com Histórico Completo**: se **QUALQUER** resposta (pagamento OU
subscrição) for rejeitada, o status muda para **REJECTED imediatamente**. Mesmo após rejeitado, quando a segunda
resposta chegar, ela é **registrada no histórico**.
...
**Testes**:
`order-component-test/src/test/java/io/github/athirson010/componenttest/validacao/ValidationRulesCompleteComponentTest.java`
...
- `APPROVED`: Marca `paymentConfirmed = true`, mantém PENDING até subscription chegar (ou APROVA se subscription já veio
  aprovada)
- `REJECTED`: Muda status para REJECTED **imediatamente** (não aguarda subscription)
- **Histórico**: Se já estiver REJECTED (por subscription), adiciona entrada no histórico registrando resultado do
  pagamento

**Implementação**:
`order-adapters-in/src/main/java/io/github/athirson010/adapters/in/messaging/rabbitmq/PaymentConfirmationConsumer.java`
...
- `APPROVED`: Marca `subscriptionConfirmed = true`, mantém PENDING até payment chegar (ou APROVA se payment já veio
  aprovado)
...
**Implementação**:
`order-adapters-in/src/main/java/io/github/athirson010/adapters/in/messaging/rabbitmq/InsuranceSubscriptionConfirmationConsumer.java`
...
O sistema possui **3 profiles** que permitem executar cada consumer de forma independente para **escalabilidade
horizontal**:
...
**Nota**: A aplicação Java não está no docker-compose (executada via Maven/JAR), permitindo maior agilidade no
...
SonarQube é uma plataforma de análise estática de código que detecta bugs, vulnerabilidades de segurança, code smells e
